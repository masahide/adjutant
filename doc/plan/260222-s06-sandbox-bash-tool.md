...
| `ADJUTANT_SANDBOX_IMAGE` | `adjutant-sandbox:bookworm-slim` | Docker イメージ名 |
...
docker create --name adjutant-sandbox-a1b2c3 \
  ...
  adjutant-sandbox:bookworm-slim sleep infinity
```
...
export function configureSandbox(
  config: { containerName: string; workdir: string; hostWorkspaceDir: string; mode: SandboxMode } | null
): void {
...
- [ ] **Test** `tests/sandbox/config.test.ts` — 環境変数からの設定解決テスト (Red)
  - `SandboxMode` 3 値（`off`/`non-main`/`all`）、不正値→`off` フォールバック
  - `containerPrefix`（旧 `containerName`）のデフォルトとオーバーライド
- [ ] **Impl** `src/sandbox/types.ts` + `src/sandbox/config.ts` — 型定義と `resolveSandboxConfig()` (Green)
  - 再利用: `src/runtime/env-parsers.ts` の `parseStringEnv`, `parsePositiveIntEnv`

### Phase 3: パス変換

- [ ] **Test** `tests/sandbox/path-mapper.test.ts` — ホスト↔コンテナパス変換テスト (Red)
- [ ] **Impl** `src/sandbox/path-mapper.ts` — `createPathMapper()` → `{ hostToContainer(path) }` (Green)

### Phase 4: Docker 操作

- [ ] **Test** `tests/sandbox/docker.test.ts` (Red)
...
- [ ] **Impl** `src/sandbox/docker.ts` — Docker CLI ラッパー群 (Green)

### Phase 5: BashOperations 実装

- [ ] **Test** `tests/sandbox/docker-bash-operations.test.ts` (Red)
  - `buildDockerExecArgs()` — `bash -lc`（`sh` ではなく `bash`）
  - `shouldSandbox()` — mode×scope マトリクス
  - 正常終了/エラー終了/タイムアウト/AbortSignal
- [ ] **Impl** `src/sandbox/docker-bash-operations.ts` — `BashOperations` 実装 (Green)

### Phase 6: ファクトリー統合

- [ ] **Impl** `src/assistant/agent-session-factory.ts` — `configureSandbox()` + `shouldSandbox()` 追加、`createCodingTools` 分岐
...
- [ ] **Impl** `src/runtime/app-runtime-config.ts` — `SandboxRuntimeConfig` 型追加
- [ ] **Impl** `src/runtime/runtime-config-loader.ts` — `resolveSandboxConfig` 呼び出し追加

### Phase 7: ライフサイクル統合

- [ ] **Impl** `src/assistant/main.ts`
  - 起動時: `mode ≠ off` → `isDockerAvailable()` → `ensureDockerImage()` → `ensureSandboxContainer(prefix, pid)` → `configureSandbox()`
  - shutdown 時: `destroySandboxContainer(containerName)` （owner label 検証付き）
- [ ] **Impl** `Dockerfile.sandbox` — `FROM debian:bookworm-slim`, `apt-get install bash git curl`, `USER 1000`
- [ ] **Impl** `package.json` — `sandbox:build` スクリプト追加

### Phase 8: 検証と文書更新

- [ ] `pnpm run check` 全パス
- [ ] `CLAUDE.md` にサンドボックス環境変数 7 項目を追記
...
- [ ] `pnpm run check` (format → typecheck → test) パス
- [ ] Docker 不要で全テストパス
- [ ] CLAUDE.md の環境変数テーブルが更新されていること
...
| `Dockerfile.sandbox` | サンドボックスイメージ（bash インストール、USER 1000） |
