...
- 破壊点
  - 旧単一ファイル cache 読込を削除する場合、既存環境は新ディレクトリ形式へ移行が必要
- 最小移行方針
  - `data/_cache/slack/*.json` 旧形式を 1 回変換するスクリプトを `hack/` に追加し、README に実行手順を記載
- 未確定事項
  - 旧形式フォールバックを今サイクルで即削除するか、1リリース猶予するか
  - notification の actor 補完を users cache のみで固定するか、bot_name 併用を許可するか
