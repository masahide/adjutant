...
| パス | R/W | 環境変数での上書き | 定義箇所 |
|------|-----|-------------------|----------|
| `{agentStateDir}/sessions.json` | R/W | `ADJUTANT_SESSION_ENTRIES_PATH` | `src/assistant/session-paths.ts:82`, `src/assistant/session-entry-store.ts:52,143` |
...
| `{agentStateDir}/summary-batch-watermark.json` | R/W | — | `src/assistant/session-paths.ts:75`, `src/assistant/markdown-summary-batch.ts:290,309` |
| `{workspaceDir}/memory/sessions/` (レガシー) | R | — | `src/assistant/session-paths.ts:103` |
...
| パス | R/W | 環境変数での上書き | 定義箇所 |
|------|-----|-------------------|----------|
| `{workspaceDir}/memory/idempotency.jsonl` | R/W | `ADJUTANT_IDEMPOTENCY_STORE_PATH` | `src/runtime/runtime-config-loader.ts:133`, `src/assistant/idempotency-store.ts:33-34` |

## 10. ハートビート実行記録

| パス | R/W | 定義箇所 |
|------|-----|----------|
| `{dataDir}/_assistant/heartbeat-runs.jsonl` | W | `src/assistant/heartbeat-runner.ts:95,124` |
...
- `{workspaceDir}/memory/sessions/` (レガシー) 配下の全 `.jsonl`
...
│       └── BOOTSTRAP.md                     [R]
├── memory/
│   └── POLICY_ROUTING.json                  [R]   ルーティングポリシー
└── data/                                    (= dataDir = workspaceDir)
    ├── MEMORY.md                            [R/W] 長期記憶
    ├── memory.md                            [R]   長期記憶 (別名)
