    ...
    return await new Promise<DockerCommandResult>((resolveResult, rejectResult) => {
      ...
      child.on("close", (code) => {
        ...
        if (exitCode !== 0 && !options?.allowFailure) {
          rejectResult(new Error(stderr.trim() || `docker ${args.join(" ")} failed`));
          return;
        }
        resolveResult({
          stdout,
...
export async function checkDockerAvailability(params?: {
  ...
  try {
    ...
    return {
      available: false,
      reason: result.stderr.trim() || result.stdout.trim() || "docker daemon unavailable",
    };
  } catch (error) {
    return {
      available: false,
...
export async function ensureDockerImage(
  ...
  throw new Error(
    `sandbox image not found: ${image}. auto-build failed: ${buildResult.stderr.trim() || buildResult.stdout.trim() || "docker build failed"}`
  );
}
