...
export function verifyJsonlChecksum(record: Record<string, unknown>): boolean {
  if (typeof record.checksum !== "string" || record.checksum.trim().length === 0) {
    return true;
  }
  const payload = { ...record };
  delete (payload as { checksum?: string }).checksum;
  return computeJsonlChecksum(payload) === record.checksum;
}
