...
- [x] GREEN: モデル一覧を `adjutant.config.json` から読み込むストアと、モック API を使った差分表示ロジックを実装する。
...
- [x] LLM モデル一覧がハードコードされており、`adjutant.config.json` の既定モデル設定が反映されない。
...
- [x] RED: LLM モデル一覧が固定配列であることをテストで露出させ、`adjutant.config.json` の既定モデルをモックして失敗させる。
